...
Zernio - the Ruby gem for the [Zernio API](https://zernio.com)

API reference for Zernio. Authenticate with a Bearer API key.
Base URL: https://zernio.com/api

> **Note:** Previously published as `late-sdk`. The old gem name and `Late` module still work for backwards compatibility.
...
> **Backwards compatibility:** `require 'late-sdk'` and `Late::PostsApi.new` still work. Both `Late` and `Zernio` module names are supported.
