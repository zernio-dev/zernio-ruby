...
module Zernio
  class EnableWhatsAppCallingLegacy200Response < ApiModelBase
    attr_accessor :success

    attr_accessor :calling_enabled

    attr_accessor :sip_hostname
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::EnableWhatsAppCallingLegacy200Response` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::EnableWhatsAppCallingLegacy200Response`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'sip_hostname')
        self.sip_hostname = attributes[:'sip_hostname']
      end
