...
module Zernio
  class SendInboxMessageRequestInteractiveActionOneOf7SectionsInner < ApiModelBase
    ...
    def self.openapi_types
      {
        :'title' => :'String',
        :'product_items' => :'Array<SendInboxMessageRequestInteractiveActionOneOf7SectionsInnerProductItemsInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::SendInboxMessageRequestInteractiveActionOneOf7SectionsInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::SendInboxMessageRequestInteractiveActionOneOf7SectionsInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
