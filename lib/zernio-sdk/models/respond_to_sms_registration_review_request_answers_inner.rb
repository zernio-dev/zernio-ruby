...
module Zernio
  class RespondToSmsRegistrationReviewRequestAnswersInner < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::RespondToSmsRegistrationReviewRequestAnswersInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::RespondToSmsRegistrationReviewRequestAnswersInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'point_id')
    ...
    # @param [Object] point_id Value to be assigned
    def point_id=(point_id)
      if point_id.nil?
        fail ArgumentError, 'point_id cannot be nil'
      end
