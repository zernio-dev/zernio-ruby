...
module Zernio
  class ConnectDiscordChannelRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::ConnectDiscordChannelRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::ConnectDiscordChannelRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @guild_id.nil?
        invalid_properties.push('invalid value for "guild_id", guild_id cannot be nil.')
      end
    ...
    # @param [Object] guild_id Value to be assigned
    def guild_id=(guild_id)
      if guild_id.nil?
        fail ArgumentError, 'guild_id cannot be nil'
      end
    ...
    def hash
      [guild_id, channel_id, profile_id].hash
    end
