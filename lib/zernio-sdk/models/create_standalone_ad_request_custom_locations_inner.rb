...
module Zernio
  class CreateStandaloneAdRequestCustomLocationsInner < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::CreateStandaloneAdRequestCustomLocationsInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::CreateStandaloneAdRequestCustomLocationsInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @distance_unit.nil?
        invalid_properties.push('invalid value for "distance_unit", distance_unit cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @distance_unit.nil?
    ...
    def hash
      [latitude, longitude, radius, distance_unit, name, address].hash
    end
