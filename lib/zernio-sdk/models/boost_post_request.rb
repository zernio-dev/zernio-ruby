...
module Zernio
  class BoostPostRequest < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'promo_codes' => :'Array<BoostPostRequestPromoCodesInner>',
    ...
    def initialize(attributes = {})
      ...
      if attributes.key?(:'spark_posts')
        if (value = attributes[:'spark_posts']).is_a?(Array)
          self.spark_posts = value
        end
      end

      if attributes.key?(:'promo_codes')
        if (value = attributes[:'promo_codes']).is_a?(Array)
    ...
    def list_invalid_properties
      ...
      if !@spark_posts.nil? && @spark_posts.length > 50
        invalid_properties.push('invalid value for "spark_posts", number of items must be less than or equal to 50.')
      end

      if !@spark_posts.nil? && @spark_posts.length < 1
        invalid_properties.push('invalid value for "spark_posts", number of items must be greater than or equal to 1.')
      end

      if !@promo_codes.nil? && @promo_codes.length > 10
        invalid_properties.push('invalid value for "promo_codes", number of items must be less than or equal to 10.')
      end

      if !@promo_codes.nil? && @promo_codes.length < 1
        invalid_properties.push('invalid value for "promo_codes", number of items must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@currency.nil? && @currency.to_s.length < 3
      return false if !@spark_posts.nil? && @spark_posts.length > 50
      return false if !@spark_posts.nil? && @spark_posts.length < 1
      return false if !@promo_codes.nil? && @promo_codes.length > 10
      return false if !@promo_codes.nil? && @promo_codes.length < 1
    ...
    # Custom attribute writer method with validation
    # @param [Object] spark_posts Value to be assigned
    def spark_posts=(spark_posts)
      if spark_posts.nil?
        fail ArgumentError, 'spark_posts cannot be nil'
      end

      if spark_posts.length > 50
        fail ArgumentError, 'invalid value for "spark_posts", number of items must be less than or equal to 50.'
      end

      if spark_posts.length < 1
        fail ArgumentError, 'invalid value for "spark_posts", number of items must be greater than or equal to 1.'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] promo_codes Value to be assigned
    def promo_codes=(promo_codes)
      if promo_codes.nil?
        fail ArgumentError, 'promo_codes cannot be nil'
      end

      if promo_codes.length > 10
        fail ArgumentError, 'invalid value for "promo_codes", number of items must be less than or equal to 10.'
      end

      if promo_codes.length < 1
        fail ArgumentError, 'invalid value for "promo_codes", number of items must be greater than or equal to 1.'
      end
    ...
    def hash
      [creative_features, post_id, platform_post_id, account_id, ad_account_id, name, campaign_name, ad_set_name, goal, ad_set_id, budget, instagram_account_id, destination_type, whatsapp_phone_number, currency, schedule, targeting, raw_targeting, bid_strategy, bid_amount, roas_average_floor, platform_specific_data, tracking, special_ad_categories, special_ad_category_country, regional_regulated_categories, regional_regulation_identities, link_url, call_to_action, spark_auth_code, smart_plus, spark_posts, promo_codes, promoted_object, dsa_beneficiary, dsa_payor, lead_gen_form_id, status, optimization_goal].hash
    end
