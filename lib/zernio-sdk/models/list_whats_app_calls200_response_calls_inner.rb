...
module Zernio
  class ListWhatsAppCalls200ResponseCallsInner < ApiModelBase
    attr_accessor :_id
    ...
    def self.openapi_types
      {
        ...
        :'billing' => :'ListWhatsAppCalls200ResponseCallsInnerBilling'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::ListWhatsAppCalls200ResponseCallsInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::ListWhatsAppCalls200ResponseCallsInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [_id, direction, from, to, status, started_at, ended_at, duration_seconds, end_reason, recording_url, billing].hash
    end
