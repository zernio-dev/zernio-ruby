  ...
  class CreateMessagingAdRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::CreateMessagingAdRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::CreateMessagingAdRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@destinations.nil? && @destinations.length > 3
        invalid_properties.push('invalid value for "destinations", number of items must be less than or equal to 3.')
      end

      if !@destinations.nil? && @destinations.length < 2
        invalid_properties.push('invalid value for "destinations", number of items must be greater than or equal to 2.')
      end
    ...
    def valid?
      ...
      destination_validator = EnumAttributeValidator.new('String', ["whatsapp", "messenger", "instagram_direct"])
      return false unless destination_validator.valid?(@destination)
      return false if !@destinations.nil? && @destinations.length > 3
      return false if !@destinations.nil? && @destinations.length < 2
      true
    end
    ...
    def destination=(destination)
      validator = EnumAttributeValidator.new('String', ["whatsapp", "messenger", "instagram_direct"])
    ...
    def hash
      [creative_features, tracking, account_id, ad_account_id, name, campaign_name, ad_set_name, platform_post_id, existing_post_id, object_story_id, page_id, whatsapp_phone_number, headline, body, image_url, video, welcome_message, creatives, ad_set_id, budget_amount, budget_type, currency, end_date, countries, cities, regions, zips, metros, country_groups, custom_locations, age_min, age_max, interests, audience_id, placements, advantage_audience, objective, status, campaign_status, bid_strategy, bid_amount, roas_average_floor, dsa_beneficiary, dsa_payor, regional_regulated_categories, regional_regulation_identities, destination, destinations].hash
    end
