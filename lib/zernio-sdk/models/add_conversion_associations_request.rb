...
module Zernio
  class AddConversionAssociationsRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::AddConversionAssociationsRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::AddConversionAssociationsRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @campaign_ids.length > 100
        invalid_properties.push('invalid value for "campaign_ids", number of items must be less than or equal to 100.')
      end
    ...
    def valid?
      ...
      return false if @campaign_ids.length > 100
    ...
    def campaign_ids=(campaign_ids)
      ...
      if campaign_ids.length > 100
        fail ArgumentError, 'invalid value for "campaign_ids", number of items must be less than or equal to 100.'
      end
    ...
    def hash
      [ad_account_id, campaign_ids].hash
    end
