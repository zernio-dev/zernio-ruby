...
module Zernio
  class ListPhoneNumbers200Response < ApiModelBase
    ...
    def self.openapi_types
      {
        :'numbers' => :'Array<ListPhoneNumbers200ResponseNumbersInner>',
        :'connected' => :'Array<ListPhoneNumbers200ResponseConnectedInner>',
        :'imessage' => :'Array<ImessageSenderLifecycle>',
        :'sandbox' => :'ListPhoneNumbers200ResponseSandbox'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::ListPhoneNumbers200Response` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::ListPhoneNumbers200Response`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'imessage')
        if (value = attributes[:'imessage']).is_a?(Array)
          self.imessage = value
        end
      end
    ...
    def hash
      [numbers, connected, imessage, sandbox].hash
    end
