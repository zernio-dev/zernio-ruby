...
module Zernio
  ...
  class LinkedInPlatformDataPoll < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::LinkedInPlatformDataPoll` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::LinkedInPlatformDataPoll`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @question.to_s.length > 140
        invalid_properties.push('invalid value for "question", the character length must be smaller than or equal to 140.')
      end
    ...
    def valid?
      ...
      return false if @question.to_s.length > 140
      return false if @question.to_s.length < 1
      return false if @options.nil?
      return false if @options.length > 4
      return false if @options.length < 2
      duration_validator = EnumAttributeValidator.new('String', ["ONE_DAY", "THREE_DAYS", "SEVEN_DAYS", "FOURTEEN_DAYS"])
    ...
    def question=(question)
      ...
      if question.to_s.length > 140
        fail ArgumentError, 'invalid value for "question", the character length must be smaller than or equal to 140.'
      end
    ...
    def duration=(duration)
      validator = EnumAttributeValidator.new('String', ["ONE_DAY", "THREE_DAYS", "SEVEN_DAYS", "FOURTEEN_DAYS"])
    ...
    def hash
      [question, options, duration].hash
    end
