...
module Zernio
  class ListAdsTikTokIdentities200ResponseIdentitiesInner < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::ListAdsTikTokIdentities200ResponseIdentitiesInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::ListAdsTikTokIdentities200ResponseIdentitiesInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      identity_type_validator = EnumAttributeValidator.new('String', ["TT_USER", "BC_AUTH_TT", "CUSTOMIZED_USER"])
    ...
    def identity_type=(identity_type)
      validator = EnumAttributeValidator.new('String', ["TT_USER", "BC_AUTH_TT", "CUSTOMIZED_USER"])
    ...
    def hash
      [identity_id, identity_type, display_name, username, profile_image, identity_authorized_bc_id].hash
    end
