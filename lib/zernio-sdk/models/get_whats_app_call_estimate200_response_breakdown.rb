...
module Zernio
  class GetWhatsAppCallEstimate200ResponseBreakdown < ApiModelBase
    attr_accessor :meta_minutes
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::GetWhatsAppCallEstimate200ResponseBreakdown` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::GetWhatsAppCallEstimate200ResponseBreakdown`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'meta_minutes')
        self.meta_minutes = attributes[:'meta_minutes']
      end
    ...
    def hash
      [meta_minutes, meta_cost_usd, telnyx_cost_usd, recording_cost_usd, billable_cost_usd, total_cost_usd].hash
    end
