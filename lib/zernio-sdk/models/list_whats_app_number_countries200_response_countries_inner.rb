...
module Zernio
  class ListWhatsAppNumberCountries200ResponseCountriesInner < ApiModelBase
    # ISO 3166-1 alpha-2
    attr_accessor :code
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::ListWhatsAppNumberCountries200ResponseCountriesInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::ListWhatsAppNumberCountries200ResponseCountriesInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [code, tier, monthly_cents, needs_kyc, outbound_calling_available].hash
    end
