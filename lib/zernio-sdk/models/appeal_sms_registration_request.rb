  ...
  class AppealSmsRegistrationRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::AppealSmsRegistrationRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::AppealSmsRegistrationRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @appeal_reason.to_s.length > 2000
        invalid_properties.push('invalid value for "appeal_reason", the character length must be smaller than or equal to 2000.')
      end

      if @appeal_reason.to_s.length < 10
    ...
    def appeal_reason=(appeal_reason)
      ...
      if appeal_reason.to_s.length > 2000
        fail ArgumentError, 'invalid value for "appeal_reason", the character length must be smaller than or equal to 2000.'
      end

      if appeal_reason.to_s.length < 10
        fail ArgumentError, 'invalid value for "appeal_reason", the character length must be greater than or equal to 10.'
      end
