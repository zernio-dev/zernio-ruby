  ...
  class CallRecord < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::CallRecord` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::CallRecord`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [_id, account_id, conversation_id, contact_id, channel, direction, from, to, forward_to, greeting, status, is_voicemail, amd, answered_machine, forward_caller_id, recording_enabled, transcription_enabled, transcription_language, started_at, answered_at, ended_at, transferred_at, duration_seconds, end_reason, hangup_cause, sip_hangup_cause, call_errors, recording_url, last_transcript_snippet, transcript, billing, created_at, updated_at].hash
    end
