...
module Zernio
  class CreateAdNegativeKeywordListRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::CreateAdNegativeKeywordListRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::CreateAdNegativeKeywordListRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@keywords.nil? && @keywords.length > 5000
        invalid_properties.push('invalid value for "keywords", number of items must be less than or equal to 5000.')
      end
    ...
    def valid?
      ...
      return false if !@keywords.nil? && @keywords.length > 5000
      true
    end
    ...
    def keywords=(keywords)
      ...
      if keywords.length > 5000
        fail ArgumentError, 'invalid value for "keywords", number of items must be less than or equal to 5000.'
      end
    ...
    def hash
      [account_id, ad_account_id, customer_id, platform, name, keywords].hash
    end
