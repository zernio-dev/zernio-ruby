=begin
#Zernio API

#API reference for Zernio. Authenticate with a Bearer API key. Base URL: https://zernio.com/api  Versioning and deprecation: all endpoints are versioned in the URL path (current version: /v1). Breaking changes only ship in a new path version; existing versions keep working. Deprecated operations are marked 'deprecated: true' in this spec and announced in the changelog (https://zernio.com/changelog) before removal.  Errors: every 4xx/5xx response is application/json with a machine-readable 'code' and a human-readable 'error' message (see the ErrorResponse schema).  Request ids: responses carry an X-Request-Id header with the id we log the request under. Quote it when reporting a problem. A valid x-request-id you send is reused as that id. 

The version of the OpenAPI document: 1.98.1
Contact: support@zernio.com
Generated by: https://openapi-generator.tech
Generator version: 7.19.0

=end

require 'date'
require 'time'

module Zernio
  class ListSmsRegistrations200ResponseRegistrationsInner < ApiModelBase
    attr_accessor :id

    attr_accessor :registration_type

    attr_accessor :display_name

    # requested/changes_requested = pre-submission review states; customers see them as pending / needs changes.
    attr_accessor :status

    # Carrier-registry brand status (e.g. VERIFIED).
    attr_accessor :brand_status

    attr_accessor :campaign_status

    # TCR brand id, useful when referencing the brand in carrier support threads.
    attr_accessor :brand_id

    # TCR campaign id.
    attr_accessor :campaign_id

    attr_accessor :decline_reason

    # Toll-free only: when the carrier requested changes (\"Waiting For Customer\"). The request must be resubmitted within 7 days of this timestamp or it expires.
    attr_accessor :tf_action_required_at

    attr_accessor :phone_numbers

    # Sole-prop 10DLC only; the OTP step is still pending.
    attr_accessor :awaiting_otp

    # The open change request as text (status changes_requested).
    attr_accessor :admin_review_note

    # When you last answered a change request.
    attr_accessor :last_response_at

    # Rejected by the carriers at least once. A pending registration with this set is our fix, back with the carriers.
    attr_accessor :previously_rejected

    # When the carriers last rejected it.
    attr_accessor :last_rejected_at

    # Rejected in our review before anything was filed with the carriers (not a carrier rejection; nothing to fix or appeal).
    attr_accessor :rejected_before_submission

    # Sole proprietor only: the verification code was never entered within 30 days. Start SMS setup again; it revives the same brand with no second brand fee.
    attr_accessor :otp_expired

    attr_accessor :review_request

    # Carrier-assigned brand trust score; drives throughput.
    attr_accessor :trust_score

    attr_accessor :throughput

    class EnumAttributeValidator
      attr_reader :datatype
      attr_reader :allowable_values

      def initialize(datatype, allowable_values)
        @allowable_values = allowable_values.map do |value|
          case datatype.to_s
          when /Integer/i
            value.to_i
          when /Float/i
            value.to_f
          else
            value
          end
        end
      end

      def valid?(value)
        !value || allowable_values.include?(value)
      end
    end

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'id' => :'id',
        :'registration_type' => :'registrationType',
        :'display_name' => :'displayName',
        :'status' => :'status',
        :'brand_status' => :'brandStatus',
        :'campaign_status' => :'campaignStatus',
        :'brand_id' => :'brandId',
        :'campaign_id' => :'campaignId',
        :'decline_reason' => :'declineReason',
        :'tf_action_required_at' => :'tfActionRequiredAt',
        :'phone_numbers' => :'phoneNumbers',
        :'awaiting_otp' => :'awaitingOtp',
        :'admin_review_note' => :'adminReviewNote',
        :'last_response_at' => :'lastResponseAt',
        :'previously_rejected' => :'previouslyRejected',
        :'last_rejected_at' => :'lastRejectedAt',
        :'rejected_before_submission' => :'rejectedBeforeSubmission',
        :'otp_expired' => :'otpExpired',
        :'review_request' => :'reviewRequest',
        :'trust_score' => :'trustScore',
        :'throughput' => :'throughput'
      }
    end

    # Returns attribute mapping this model knows about
    def self.acceptable_attribute_map
      attribute_map
    end

    # Returns all the JSON keys this model knows about
    def self.acceptable_attributes
      acceptable_attribute_map.values
    end

    # Attribute type mapping.
    def self.openapi_types
      {
        :'id' => :'String',
        :'registration_type' => :'String',
        :'display_name' => :'String',
        :'status' => :'String',
        :'brand_status' => :'String',
        :'campaign_status' => :'String',
        :'brand_id' => :'String',
        :'campaign_id' => :'String',
        :'decline_reason' => :'String',
        :'tf_action_required_at' => :'Time',
        :'phone_numbers' => :'Array<String>',
        :'awaiting_otp' => :'Boolean',
        :'admin_review_note' => :'String',
        :'last_response_at' => :'Time',
        :'previously_rejected' => :'Boolean',
        :'last_rejected_at' => :'Time',
        :'rejected_before_submission' => :'Boolean',
        :'otp_expired' => :'Boolean',
        :'review_request' => :'SmsRegistrationReviewRequest',
        :'trust_score' => :'Float',
        :'throughput' => :'ListSmsRegistrations200ResponseRegistrationsInnerThroughput'
      }
    end

    # List of attributes with nullable: true
    def self.openapi_nullable
      Set.new([
        :'display_name',
        :'brand_id',
        :'campaign_id',
        :'decline_reason',
        :'tf_action_required_at',
        :'admin_review_note',
        :'last_response_at',
        :'last_rejected_at',
        :'review_request',
        :'trust_score',
      ])
    end

    # Initializes the object
    # @param [Hash] attributes Model attributes in the form of hash
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::ListSmsRegistrations200ResponseRegistrationsInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      acceptable_attribute_map = self.class.acceptable_attribute_map
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::ListSmsRegistrations200ResponseRegistrationsInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'id')
        self.id = attributes[:'id']
      end

      if attributes.key?(:'registration_type')
        self.registration_type = attributes[:'registration_type']
      end

      if attributes.key?(:'display_name')
        self.display_name = attributes[:'display_name']
      end

      if attributes.key?(:'status')
        self.status = attributes[:'status']
      end

      if attributes.key?(:'brand_status')
        self.brand_status = attributes[:'brand_status']
      end

      if attributes.key?(:'campaign_status')
        self.campaign_status = attributes[:'campaign_status']
      end

      if attributes.key?(:'brand_id')
        self.brand_id = attributes[:'brand_id']
      end

      if attributes.key?(:'campaign_id')
        self.campaign_id = attributes[:'campaign_id']
      end

      if attributes.key?(:'decline_reason')
        self.decline_reason = attributes[:'decline_reason']
      end

      if attributes.key?(:'tf_action_required_at')
        self.tf_action_required_at = attributes[:'tf_action_required_at']
      end

      if attributes.key?(:'phone_numbers')
        if (value = attributes[:'phone_numbers']).is_a?(Array)
          self.phone_numbers = value
        end
      end

      if attributes.key?(:'awaiting_otp')
        self.awaiting_otp = attributes[:'awaiting_otp']
      end

      if attributes.key?(:'admin_review_note')
        self.admin_review_note = attributes[:'admin_review_note']
      end

      if attributes.key?(:'last_response_at')
        self.last_response_at = attributes[:'last_response_at']
      end

      if attributes.key?(:'previously_rejected')
        self.previously_rejected = attributes[:'previously_rejected']
      end

      if attributes.key?(:'last_rejected_at')
        self.last_rejected_at = attributes[:'last_rejected_at']
      end

      if attributes.key?(:'rejected_before_submission')
        self.rejected_before_submission = attributes[:'rejected_before_submission']
      end

      if attributes.key?(:'otp_expired')
        self.otp_expired = attributes[:'otp_expired']
      end

      if attributes.key?(:'review_request')
        self.review_request = attributes[:'review_request']
      end

      if attributes.key?(:'trust_score')
        self.trust_score = attributes[:'trust_score']
      end

      if attributes.key?(:'throughput')
        self.throughput = attributes[:'throughput']
      end
    end

    # Show invalid properties with the reasons. Usually used together with valid?
    # @return Array for valid properties with the reasons
    def list_invalid_properties
      warn '[DEPRECATED] the `list_invalid_properties` method is obsolete'
      invalid_properties = Array.new
      invalid_properties
    end

    # Check to see if the all the properties in the model are valid
    # @return true if the model is valid
    def valid?
      warn '[DEPRECATED] the `valid?` method is obsolete'
      registration_type_validator = EnumAttributeValidator.new('String', ["standard_10dlc", "sole_prop_10dlc", "toll_free"])
      return false unless registration_type_validator.valid?(@registration_type)
      status_validator = EnumAttributeValidator.new('String', ["pending", "approved", "rejected", "requested", "changes_requested", "deactivated"])
      return false unless status_validator.valid?(@status)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] registration_type Object to be assigned
    def registration_type=(registration_type)
      validator = EnumAttributeValidator.new('String', ["standard_10dlc", "sole_prop_10dlc", "toll_free"])
      unless validator.valid?(registration_type)
        fail ArgumentError, "invalid value for \"registration_type\", must be one of #{validator.allowable_values}."
      end
      @registration_type = registration_type
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] status Object to be assigned
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["pending", "approved", "rejected", "requested", "changes_requested", "deactivated"])
      unless validator.valid?(status)
        fail ArgumentError, "invalid value for \"status\", must be one of #{validator.allowable_values}."
      end
      @status = status
    end

    # Checks equality by comparing each attribute.
    # @param [Object] Object to be compared
    def ==(o)
      return true if self.equal?(o)
      self.class == o.class &&
          id == o.id &&
          registration_type == o.registration_type &&
          display_name == o.display_name &&
          status == o.status &&
          brand_status == o.brand_status &&
          campaign_status == o.campaign_status &&
          brand_id == o.brand_id &&
          campaign_id == o.campaign_id &&
          decline_reason == o.decline_reason &&
          tf_action_required_at == o.tf_action_required_at &&
          phone_numbers == o.phone_numbers &&
          awaiting_otp == o.awaiting_otp &&
          admin_review_note == o.admin_review_note &&
          last_response_at == o.last_response_at &&
          previously_rejected == o.previously_rejected &&
          last_rejected_at == o.last_rejected_at &&
          rejected_before_submission == o.rejected_before_submission &&
          otp_expired == o.otp_expired &&
          review_request == o.review_request &&
          trust_score == o.trust_score &&
          throughput == o.throughput
    end

    # @see the `==` method
    # @param [Object] Object to be compared
    def eql?(o)
      self == o
    end

    # Calculates hash code according to all attributes.
    # @return [Integer] Hash code
    def hash
      [id, registration_type, display_name, status, brand_status, campaign_status, brand_id, campaign_id, decline_reason, tf_action_required_at, phone_numbers, awaiting_otp, admin_review_note, last_response_at, previously_rejected, last_rejected_at, rejected_before_submission, otp_expired, review_request, trust_score, throughput].hash
    end

    # Builds the object from hash
    # @param [Hash] attributes Model attributes in the form of hash
    # @return [Object] Returns the model itself
    def self.build_from_hash(attributes)
      return nil unless attributes.is_a?(Hash)
      attributes = attributes.transform_keys(&:to_sym)
      transformed_hash = {}
      openapi_types.each_pair do |key, type|
        if attributes.key?(attribute_map[key]) && attributes[attribute_map[key]].nil?
          transformed_hash["#{key}"] = nil
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[attribute_map[key]].is_a?(Array)
            transformed_hash["#{key}"] = attributes[attribute_map[key]].map { |v| _deserialize($1, v) }
          end
        elsif !attributes[attribute_map[key]].nil?
          transformed_hash["#{key}"] = _deserialize(type, attributes[attribute_map[key]])
        end
      end
      new(transformed_hash)
    end

    # Returns the object in the form of hash
    # @return [Hash] Returns the object in the form of hash
    def to_hash
      hash = {}
      self.class.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = self.class.openapi_nullable.include?(attr)
          next if !is_nullable || (is_nullable && !instance_variable_defined?(:"@#{attr}"))
        end

        hash[param] = _to_hash(value)
      end
      hash
    end

  end

end
