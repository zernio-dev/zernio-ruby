...
module Zernio
  ...
  class GetLeadForm200ResponseForm < ApiModelBase
    attr_accessor :id
    ...
    def self.openapi_types
      {
        ...
        :'questions' => :'Array<GetLeadForm200ResponseFormQuestionsInner>',
        :'thank_you_page' => :'GetLeadForm200ResponseFormThankYouPage',
        :'context_card' => :'Object'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::GetLeadForm200ResponseForm` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::GetLeadForm200ResponseForm`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, name, status, locale, created_time, leads_count, privacy_policy_url, follow_up_action_url, questions, thank_you_page, context_card].hash
    end
