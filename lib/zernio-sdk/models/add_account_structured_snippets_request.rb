...
module Zernio
  class AddAccountStructuredSnippetsRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::AddAccountStructuredSnippetsRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::AddAccountStructuredSnippetsRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @structured_snippets.nil?
        invalid_properties.push('invalid value for "structured_snippets", structured_snippets cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @structured_snippets.nil?
      return false if @structured_snippets.length > 20
      return false if @structured_snippets.length < 1
      true
    end
    ...
    def hash
      [account_id, ad_account_id, customer_id, structured_snippets].hash
    end
