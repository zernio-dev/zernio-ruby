...
module Zernio
  class AddAccountStructuredSnippets201ResponseStructuredSnippetsInner < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::AddAccountStructuredSnippets201ResponseStructuredSnippetsInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::AddAccountStructuredSnippets201ResponseStructuredSnippetsInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@values.nil? && @values.length > 10
    ...
    def valid?
      ...
      return false if !@values.nil? && @values.length > 10
      return false if !@values.nil? && @values.length < 3
      true
    end
