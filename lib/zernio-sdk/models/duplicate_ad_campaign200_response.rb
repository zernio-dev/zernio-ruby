...
module Zernio
  class DuplicateAdCampaign200Response < ApiModelBase
    # Platform ID of the new campaign
    attr_accessor :copied_campaign_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::DuplicateAdCampaign200Response` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::DuplicateAdCampaign200Response`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'copied_campaign_id')
        self.copied_campaign_id = attributes[:'copied_campaign_id']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          copied_campaign_id == o.copied_campaign_id &&
    ...
    def hash
      [copied_campaign_id, discovery, raw].hash
    end
