...
module Zernio
  class UpdateAccountStructuredSnippetsRequest < ApiModelBase
    ...
    def self.openapi_types
      {
        :'account_id' => :'String',
        :'customer_id' => :'String',
        :'updates' => :'Array<UpdateAccountStructuredSnippetsRequestUpdatesInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::UpdateAccountStructuredSnippetsRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::UpdateAccountStructuredSnippetsRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @updates.length > 20
        invalid_properties.push('invalid value for "updates", number of items must be less than or equal to 20.')
      end
    ...
    def valid?
      ...
      return false if @updates.length > 20
    ...
    def updates=(updates)
      ...
      if updates.length > 20
        fail ArgumentError, 'invalid value for "updates", number of items must be less than or equal to 20.'
      end
    ...
    def hash
      [account_id, customer_id, updates].hash
    end
