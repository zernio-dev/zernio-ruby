...
module Zernio
  class AddAccountSitelinksRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zernio::AddAccountSitelinksRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zernio::AddAccountSitelinksRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @sitelinks.nil?
        invalid_properties.push('invalid value for "sitelinks", sitelinks cannot be nil.')
      end
      ...
      if @sitelinks.length < 1
        invalid_properties.push('invalid value for "sitelinks", number of items must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @sitelinks.nil?
      return false if @sitelinks.length > 20
      return false if @sitelinks.length < 1
      true
    end
    ...
    def sitelinks=(sitelinks)
      ...
      if sitelinks.length < 1
        fail ArgumentError, 'invalid value for "sitelinks", number of items must be greater than or equal to 1.'
      end
    ...
    def hash
      [account_id, ad_account_id, customer_id, sitelinks].hash
    end
