...
module Zernio
  class AnalyticsApi
    ...
    # @option opts [String] :sort_by Sort by date, engagement, or a specific metric. Platform-specific metrics (follows, reposts, reels_skip_rate, ig_reels_*, completion_rate, profile_views, website_clicks) sort a null value as 0. (default to 'date')
    ...
    def get_analytics_with_http_info(opts = {})
      ...
      allowable_values = ["date", "engagement", "impressions", "reach", "likes", "comments", "shares", "saves", "clicks", "views", "follows", "ig_reels_avg_watch_time", "ig_reels_video_view_total_time", "reposts", "reels_skip_rate", "completion_rate", "profile_views", "website_clicks"]
    ...
    # @option opts [String] :account_id Filter by account ID. Omit for all accounts.
    ...
    # @option opts [Date] :from_date Start date (YYYY-MM-DD). Defaults to 30 days ago.
    # @option opts [Date] :to_date End date (YYYY-MM-DD). Defaults to today.
    # @option opts [Date] :since Alias of fromDate, kept for existing callers
    # @option opts [Date] :_until Alias of toDate, kept for existing callers
    ...
    # @option opts [Date] :from_date Start date (YYYY-MM-DD). Defaults to 30 days ago. Max 18 months back.
    # @option opts [Date] :to_date End date (YYYY-MM-DD). Defaults to today.
    # @option opts [Date] :start_date Alias of fromDate, kept for existing callers
    # @option opts [Date] :end_date Alias of toDate, kept for existing callers
    ...
    # @option opts [Date] :from_date Start date (YYYY-MM-DD). If omitted, returns lifetime analytics.
    # @option opts [Date] :to_date End date (YYYY-MM-DD, exclusive). Defaults to today if omitted.
    # @option opts [Date] :start_date Alias of fromDate, kept for existing callers
    # @option opts [Date] :end_date Alias of toDate, kept for existing callers
    ...
    # @option opts [Date] :from_date Start date (YYYY-MM-DD). Defaults to 30 days ago.
    # @option opts [Date] :to_date End date (YYYY-MM-DD). Defaults to today. YouTube Analytics has a 2-3 day delay, so the fetch is internally clamped to 3 days ago; any requested range extending beyond that returns zero values for the tail days. The response&#39;s dateRange.until field reflects your requested value. 
    # @option opts [Date] :since Alias of fromDate, kept for existing callers
    # @option opts [Date] :_until Alias of toDate, kept for existing callers
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [Date] :from_date Start date (YYYY-MM-DD). Defaults to 30 days ago.
    # @option opts [Date] :to_date End date (YYYY-MM-DD). Defaults to 3 days ago, the newest fully finalized day (YouTube finalizes analytics with a ~3-day delay). An explicit toDate is honored up to today: days inside the delay window are provisional and may still be revised by YouTube (see provisionalSince in the response), and days YouTube has not processed yet are omitted from dailyViews. 
    # @option opts [Date] :start_date Alias of fromDate, kept for existing callers
    # @option opts [Date] :end_date Alias of toDate, kept for existing callers
    ...
    # @option opts [Date] :from_date Start date in YYYY-MM-DD format. Defaults to 90 days ago, or to the video&#39;s publish date (lifetime) when videoId is provided. 
    # @option opts [Date] :to_date End date (YYYY-MM-DD). Defaults to 3 days ago, the newest fully finalized day (YouTube finalizes analytics with a ~3-day delay). An explicit toDate is honored up to today: days inside the delay window are provisional and may still be revised by YouTube (see provisionalSince in the response). 
    # @option opts [Date] :start_date Alias of fromDate, kept for existing callers
    # @option opts [Date] :end_date Alias of toDate, kept for existing callers
    ...
    # @param account_id [String] The Zernio account ID for the YouTube account
    # @param [Hash] opts the optional parameters
    # @option opts [Date] :from_date Start date (YYYY-MM-DD). Defaults to the video&#39;s publish date (lifetime curve).
    # @option opts [Date] :to_date End date (YYYY-MM-DD). Defaults to 3 days ago, the newest fully finalized day (YouTube finalizes analytics with a ~3-day delay). An explicit toDate is honored up to today: days inside the delay window are provisional and may still be revised by YouTube (see provisionalSince in the response). 
    # @option opts [Date] :start_date Alias of fromDate, kept for existing callers
    # @option opts [Date] :end_date Alias of toDate, kept for existing callers
