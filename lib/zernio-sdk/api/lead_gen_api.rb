...
module Zernio
  class LeadGenApi
    ...
    # @param form_id [String] 
    # @param account_id [String] The facebook or metaads account whose Page owns the form.
    # @param [Hash] opts the optional parameters
    # @option opts [String] :lead_id The test lead id returned by createTestLead (or shown in the Testing Tool). Omitted &#x3D; the test lead currently on the form.
    # @return [DeleteTestLead200Response]
    def delete_test_lead(form_id, account_id, opts = {})
      data, _status_code, _headers = delete_test_lead_with_http_info(form_id, account_id, opts)
      data
    end
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [String] :lead_id The test lead id returned by createTestLead (or shown in the Testing Tool). Omitted &#x3D; the test lead currently on the form.
    # @return [Array<(DeleteTestLead200Response, Integer, Hash)>] DeleteTestLead200Response data, response status code and response headers
    def delete_test_lead_with_http_info(form_id, account_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: LeadGenApi.delete_test_lead ...'
      end
      # verify the required parameter 'form_id' is set
      if @api_client.config.client_side_validation && form_id.nil?
        fail ArgumentError, "Missing the required parameter 'form_id' when calling LeadGenApi.delete_test_lead"
      end
      # verify the required parameter 'account_id' is set
      if @api_client.config.client_side_validation && account_id.nil?
        fail ArgumentError, "Missing the required parameter 'account_id' when calling LeadGenApi.delete_test_lead"
      end
      # resource path
      local_var_path = '/v1/ads/lead-forms/{formId}/test-leads'.sub('{' + 'formId' + '}', CGI.escape(form_id.to_s))
      ...
      query_params = opts[:query_params] || {}
      query_params[:'accountId'] = account_id
      query_params[:'leadId'] = opts[:'lead_id'] if !opts[:'lead_id'].nil?
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'DeleteTestLead200Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['bearerAuth']

      new_options = opts.merge(
        :operation => :"LeadGenApi.delete_test_lead",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
      ...
      data, status_code, headers = @api_client.call_api(:DELETE, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: LeadGenApi#delete_test_lead\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
