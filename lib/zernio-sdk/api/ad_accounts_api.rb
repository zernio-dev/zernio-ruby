...
module Zernio
  class AdAccountsApi
    ...
    # @param account_id [String] Zernio Meta Ads, Facebook or Instagram SocialAccount ID.
    # @param [Hash] opts the optional parameters
    # @option opts [String] :ad_account_id Meta ad account ID including the act_ prefix. Narrows identity resolution to the Instagram accounts reachable from this ad account.
    # @option opts [String] :ig_user_id Instagram identity to list. Must be one this connection can reach, otherwise the request is rejected with a 400.
    # @option opts [Integer] :limit Number of posts to return per page. (default to 25)
    # @option opts [String] :after Opaque Meta cursor from a previous response&#39;s paging.after.
    # @return [ListAdsInstagramPosts200Response]
    def list_ads_instagram_posts(account_id, opts = {})
      data, _status_code, _headers = list_ads_instagram_posts_with_http_info(account_id, opts)
      data
    end
    ...
    # @option opts [Integer] :limit Number of posts to return per page. (default to 25)
    # @option opts [String] :after Opaque Meta cursor from a previous response&#39;s paging.after.
    # @return [Array<(ListAdsInstagramPosts200Response, Integer, Hash)>] ListAdsInstagramPosts200Response data, response status code and response headers
    def list_ads_instagram_posts_with_http_info(account_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: AdAccountsApi.list_ads_instagram_posts ...'
      end
      # verify the required parameter 'account_id' is set
      if @api_client.config.client_side_validation && account_id.nil?
        fail ArgumentError, "Missing the required parameter 'account_id' when calling AdAccountsApi.list_ads_instagram_posts"
      end
      pattern = Regexp.new(/^act_[0-9]+$/)
      if @api_client.config.client_side_validation && !opts[:'ad_account_id'].nil? && opts[:'ad_account_id'] !~ pattern
        fail ArgumentError, "invalid value for 'opts[:\"ad_account_id\"]' when calling AdAccountsApi.list_ads_instagram_posts, must conform to the pattern #{pattern}."
      end

      if @api_client.config.client_side_validation && !opts[:'limit'].nil? && opts[:'limit'] > 100
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling AdAccountsApi.list_ads_instagram_posts, must be smaller than or equal to 100.'
      end

      if @api_client.config.client_side_validation && !opts[:'limit'].nil? && opts[:'limit'] < 1
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling AdAccountsApi.list_ads_instagram_posts, must be greater than or equal to 1.'
      end

      # resource path
      local_var_path = '/v1/ads/instagram-posts'

      # query parameters
      query_params = opts[:query_params] || {}
      query_params[:'accountId'] = account_id
      query_params[:'adAccountId'] = opts[:'ad_account_id'] if !opts[:'ad_account_id'].nil?
      query_params[:'igUserId'] = opts[:'ig_user_id'] if !opts[:'ig_user_id'].nil?
      query_params[:'limit'] = opts[:'limit'] if !opts[:'limit'].nil?
      query_params[:'after'] = opts[:'after'] if !opts[:'after'].nil?
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'ListAdsInstagramPosts200Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['bearerAuth']

      new_options = opts.merge(
        :operation => :"AdAccountsApi.list_ads_instagram_posts",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: AdAccountsApi#list_ads_instagram_posts\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
