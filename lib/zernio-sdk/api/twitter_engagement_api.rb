...
module Zernio
  class TwitterEngagementApi
    ...
    # @param tweet_id [String] Numeric tweet ID or a tweet URL (e.g. https://x.com/user/status/123...). The same name the other /v1/twitter operations use (retweet, bookmark).
    # @param [Hash] opts the optional parameters
    # @option opts [String] :id Alias of tweetId, kept for existing callers
    # @return [GetTweet200Response]
    def get_tweet(account_id, tweet_id, opts = {})
      data, _status_code, _headers = get_tweet_with_http_info(account_id, tweet_id, opts)
      data
    end
    ...
    def get_tweet_with_http_info(account_id, tweet_id, opts = {})
      if @api_client.config.debugging
      ...
      if @api_client.config.client_side_validation && tweet_id.nil?
        fail ArgumentError, "Missing the required parameter 'tweet_id' when calling TwitterEngagementApi.get_tweet"
      end
      # resource path
