...
module Zernio
  class WebhooksApi
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [String] :webhook_id Webhook ID to delete, the same name the other /v1/webhooks operations use (logs, redeliver, test). Required unless the deprecated &#x60;id&#x60; is sent instead.
    # @option opts [String] :id Alias of webhookId, kept for existing callers
    # @return [UpdateYoutubeDefaultPlaylist200Response]
    def delete_webhook_settings(opts = {})
      data, _status_code, _headers = delete_webhook_settings_with_http_info(opts)
      data
    end
    ...
    def delete_webhook_settings_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: WebhooksApi.delete_webhook_settings ...'
      end
      pattern = Regexp.new(/^[a-fA-F0-9]{24}$/)
      if @api_client.config.client_side_validation && !opts[:'webhook_id'].nil? && opts[:'webhook_id'] !~ pattern
        fail ArgumentError, "invalid value for 'opts[:\"webhook_id\"]' when calling WebhooksApi.delete_webhook_settings, must conform to the pattern #{pattern}."
      end

      pattern = Regexp.new(/^[a-fA-F0-9]{24}$/)
      if @api_client.config.client_side_validation && !opts[:'id'].nil? && opts[:'id'] !~ pattern
        fail ArgumentError, "invalid value for 'opts[:\"id\"]' when calling WebhooksApi.delete_webhook_settings, must conform to the pattern #{pattern}."
      end
