...
module Zernio
  class AdLibraryApi
    ...
    # @option opts [String] :account_id Zernio SocialAccount id. Required for LinkedIn (linkedin / linkedinads: its token searches). Optional for Meta, where any facebook / instagram / metaads account only selects the platform.
    ...
    # @option opts [Date] :since Meta only. Earliest delivery date (YYYY-MM-DD). LinkedIn&#39;s archive does not filter by date, so it is a 400 there: filter on details.adStatistics.firstImpressionAt / latestImpressionAt instead (EU-delivered ads only).
    # @option opts [Date] :_until Meta only. Latest delivery date (YYYY-MM-DD); a 400 on LinkedIn, see since.
